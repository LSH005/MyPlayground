...
public class TrackerMovement : MonoBehaviour
{
    ...
    private void SkipNodeTracking()
    {
        ...
        if (!isObstaclesFound)
        {
            Debug.Log($"{trackingNodeIndex-1}번 길 (ID {allRoadNodes[trackingNodeIndex-1].id}) 노드에서" +
                $"{trackingNodeIndex}번 길 (ID {allRoadNodes[trackingNodeIndex].id}) 노드로 가는 도중 " +
                $"{trackingNodeIndex+1}번 길 (ID {allRoadNodes[trackingNodeIndex+1].id}) 노드로 가는 직선을 찾음.");
